...
class MinecraftLauncher:
    ...
    def update_modpack(self):
        ...
        try:
            ...
            total_files = (
                len(manifest.get("mods", [])) + 
                len(manifest.get("config", [])) + 
                len(manifest.get("resourcepacks", [])) +
                len(manifest.get("shaderpacks", []))
            )
            ...
            # 1. Aggiorna le mod
            if "mods" in manifest:
                self.log(f"Controllo {len(manifest['mods'])} mod...")
                for mod in manifest["mods"]:
                    current_file += 1
                    self.process_file(mod, self.modpack_folder, "mod", current_file, total_files)
                self.clean_old_files(manifest.get("mods", []), self.modpack_folder, ".jar")
            
            # 2. Aggiorna i config
            if "config" in manifest:
                self.log(f"Controllo {len(manifest['config'])} file di configurazione...")
                for config in manifest["config"]:
                    current_file += 1
                    # I config possono essere in sottocartelle
                    target_path = os.path.join(self.config_folder, config.get("path", ""))
                    Path(os.path.dirname(target_path) if config.get("path") else self.config_folder).mkdir(parents=True, exist_ok=True)
                    self.process_file(config, self.config_folder, "config", current_file, total_files)
            
            # 3. Aggiorna i resourcepack
            if "resourcepacks" in manifest:
                self.log(f"Controllo {len(manifest['resourcepacks'])} resourcepack...")
                for rp in manifest["resourcepacks"]:
                    current_file += 1
                    self.process_file(rp, self.resourcepacks_folder, "resourcepack", current_file, total_files)
                self.clean_old_files(manifest.get("resourcepacks", []), self.resourcepacks_folder, ".zip")
            
            # 4. Aggiorna gli shaderpack
            if "shaderpacks" in manifest:
                self.log(f"Controllo {len(manifest['shaderpacks'])} shaderpack...")
                for sp in manifest["shaderpacks"]:
                    current_file += 1
                    self.process_file(sp, self.shaderpacks_folder, "shaderpack", current_file, total_files)
                self.clean_old_files(manifest.get("shaderpacks", []), self.shaderpacks_folder, ".zip")
    ...
    def process_file(self, file_info, target_folder, file_type, current, total):
        ...
        # Gestisci path relativi per i config
        ...
        if os.path.exists(file_path) and expected_hash:
            ...
            if current_hash == expected_hash:
                ...
                self.log(f"✓ {file_name} già aggiornato", "SUCCESS")
    ...
    def get_modpack_manifest(self):
        ...
        # Manifest di esempio per test locale
        return {
            "mods": [],
            "config": [],
            "resourcepacks": [],
